...
import { withStyles } from '@material-ui/core/styles' // eslint-disable-line import/no-unresolved
...
function withShlaky(WrappedComponent, { styles } = {}) {
  ...
  class WithShlaky extends WrappedComponent {
    ...
    attachProperties(props, context) {
      ...
      facade.config.resources.getAll().forEach(({ key }) => {
        properties[key] = facade[key]
      })
      facade.config.states.getAll().forEach(({ key }) => {
        properties[key] = facade[key]
      })
